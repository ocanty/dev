...
class TargetTS implements Target {
    ...
    async emit() {
        ...
        this.emitFilesManifest("./gluedfiles/ts/manifest.ts")
        ...
        for (const [nsId, ns] of Object.entries(this.s.namespaces)) {
            ...
            // Files
            let fCtx = this.emitCtx(`./gluedfiles/ts/${nsId}.large.ts`, "gluedfiles", nsId, ns)
    ...
    private emitFilesManifest(path: string) {
        let manifestEm = new Emitter(this.w, path)
        for (const [nsId, ns] of Object.entries(this.s.namespaces)) {
            manifestEm.line(`export * as ${title(ns.name)} from "./${nsId}.large"`)
            manifestEm.line(`export * as _ns_${nsId} from "./${nsId}.large"`)
        }
        manifestEm.close()
    }
    ...
    private async emitFiles(c: TSEmitCtx, nsId: string, ns: Namespace) {
        ...
        const files = fs.readdirSync(ns.files)
        c.em.line(`import { FileMeta } from "../file"`)
        c.em.line(`import b64 from "../b64"`)
        c.em.line(`const files: Record<string, FileMeta> = {`)
        c.em.indent()

        for (const f of files) {
            const filePath = path.join(ns.files, f)
            ...
            c.em.line(`data: new Int8Array(b64.decodeArrayBuffer("`)
...
class Emitter {
    ...
    constructor(public readonly w: Writer, private readonly path: string) {
        if (path.endsWith(".ts")) {
            this.line("/* eslint-disable */", false)
            this.line("/* This file has been automatically generated */")
        }
    }
